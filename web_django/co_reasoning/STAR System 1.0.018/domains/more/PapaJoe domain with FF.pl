...
%erotisi 6
% p(102):: animal(X),person(Person) implies
% motive(walking(Person,forest),catch(X)).
...
% p(53):: person(Person) implies
% motive(walking(Person,forest),hearBirdsChirp).
p(52) >> p(53).

p(54):: longWalk(Person,Place) implies -motive(walking(Person,Place),practiceShooting).
% p(55):: person(Person) implies
% motive(walking(Person,forest),practiceShooting).
p(54) >> p(55).
