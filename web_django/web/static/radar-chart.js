    ...
    function radar(selection) {
      selection.each(function(data) {
        ...
        var allAxis = data[0].axes.map(function(i, j) {
                            return {name: i.axis, xOffset: (i.xOffset)?i.xOffset:custoff, yOffset: (i.yOffset)?i.yOffset:custoff};
                        });
        var allAxis_vals = {}
        data[1].axes.map(function(i, j) {
                            allAxis_vals[i.axis] = i.value
                        });
            console.log('make a pure dict.')
            console.log(allAxis_vals)
        ...
        if(cfg.axisLine || cfg.axisText) {
          ...
          if(cfg.axisText) {
            newAxis.append('text').attr('class','scale-label');
            newAxis.append('text').attr('class', 'scale-value');
          }
          ...
          if(cfg.axisText) {
            ...
            .text(function(d) {
                var fixed_val = parseFloat(allAxis_vals[d.name].toFixed(1))
                var html = `${d.name}`
                // console.log(`D value is`); console.log(allAxis_vals)
                 return html; })
            ...
            .attr('dy', function(d, i) {
              var p = getVerticalPosition(i, 0.5);
              return ((p < 0.1) ? '0.5em' : ((p > 0.9) ? '0' : '1.5em'));
            })
